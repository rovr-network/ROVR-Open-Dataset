...
void Preprocess::rsm1_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  // std::cout << "start rsm1_handler" << std::endl;
  pl_surf.clear();
  pl_full.clear();
  int valid_num = 0;
  pcl::PointCloud<rovr::PointXYZIT> pl_orig;
  ...
  for (uint i = 1; i < plsize; i++)
  {
    ...
    if (1) {
      ...
      valid_num++;
      if (valid_num % point_filter_num == 0 &&
          !isnan(pl_full[i].x) && !isnan(pl_full[i].y) && !isnan(pl_full[i].z))
      {
        ...
        pl_full[i].curvature = pl_orig.points[i].timestamp; // use curvature as time of each laser points, curvature unit: ms
        // std::cout << "pl_orig[i].timestamp: " << std::setprecision(16) << pl_orig.points[i].timestamp << std::endl;
        // std::cout << "pl_full[i].curvature: " << std::setprecision(16) << pl_full[i].curvature << std::endl;
